...
// Set (key is the same as value, often just _)
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  allDogs.forEach(function (dog, i) {
    console.log(
      `Dog 🐶 number ${i + 1} is ${dog >= 3 ? `is an adult, and is ${dog} years old` : 'still a puppy'} `,
    );
  });
};
...
const calcAverageHumanAge = function (ages) {
  ...
  // Average of 2 3, (2+3)/2 = 2.5 === 2/3+3/2 = 2.5
...
const findFor = function (accounts) {
...
breeds.forEach(function (el) {
  if (el.activities.includes('running') && el.activities.includes('fetch')) {
    dogBothActivities = el.breed;
  }
});
...
let allActivities = [];
breeds.forEach(function (el) {
  allActivities = allActivities.concat(el.activities);
});
...
// 5.
let swimmingAdjacentTemp = [];
breeds.forEach(function (el) {
  if (el.activities.includes('swimming')) {
    swimmingAdjacentTemp = swimmingAdjacentTemp.concat(el.activities);
  }
});
const swimmingAdjacent = new Set(swimmingAdjacentTemp);
...
console.log(movements);
const groupedMovements = Object.groupBy(movements, movment =>
  movment > 0 ? 'deposits' : 'withdrawls',
);
